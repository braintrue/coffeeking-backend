...
# Skip the entire test suite early if runtime dependencies are unavailable.
try:
    from fastapi.testclient import TestClient
    from sqlalchemy import create_engine
    from sqlalchemy.orm import sessionmaker
except ModuleNotFoundError as exc:  # pragma: no cover - safety for constrained environments
    pytest.skip(f"Skipping tests because dependencies are missing: {exc}", allow_module_level=True)
